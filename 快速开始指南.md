# 智能巡检系统 - 快速开始指南

## 📋 前置要求

- Python 3.10 或 3.11
- 已安装虚拟环境（venv已创建）
- 阿里云百炼API密钥（DashScope）

## 🚀 快速开始（5分钟部署）

### 步骤1: 安装依赖包

```bash
# Windows
.\venv\Scripts\activate
pip install -r requirements.txt

# Linux/Mac
source venv/bin/activate
pip install -r requirements.txt
```

### 步骤2: 配置环境变量

复制示例配置文件：

```bash
# Windows
copy env.example .env

# Linux/Mac
cp env.example .env
```

编辑 `.env` 文件，填入你的阿里云API密钥：

```bash
DASHSCOPE_API_KEY=sk-你的实际密钥
```

### 步骤3: 启动BentoML服务

#### Windows:
```bash
cd api_server
start_server.bat
```

#### Linux/Mac:
```bash
cd api_server
chmod +x start_server.sh
./start_server.sh
```

#### 或者手动启动:
```bash
cd api_server
python start_server.py serve
```

服务将在 **http://0.0.0.0:3000** 启动。

### 步骤4: 测试服务

**方法1: 浏览器访问健康检查**

打开浏览器访问: http://localhost:3000/health

**方法2: 使用curl测试**

```bash
curl http://localhost:3000/health
```

**方法3: 运行测试脚本**

```bash
cd api_server
python test_service.py
```

## 📡 API使用示例

### 健康检查

```bash
curl http://localhost:3000/health
```

响应:
```json
{
    "status": "healthy",
    "service": "inspection_api_service",
    "processors": ["task1_pointer_reader", "task2_temperature", "task3_smoke_a", "task4_smoke_b"],
    "timestamp": "2024-11-21 14:30:00"
}
```

### 任务1: 指针仪表读数

```bash
curl -X POST http://localhost:3000/api/process \
  -F "image=@test_image.jpg" \
  -F "task_type=1" \
  -F "station_id=3"
```

### 任务2: 高温物体检测

```bash
curl -X POST http://localhost:3000/api/process \
  -F "image=@thermal_image.jpg" \
  -F "task_type=2" \
  -F "station_id=5" \
  -F 'params={"max_temperature":75.5,"avg_temperature":45.2,"ambient_temperature":25.0}'
```

### 任务3: 烟雾判断A

```bash
curl -X POST http://localhost:3000/api/process \
  -F "image=@smoke_image.jpg" \
  -F "task_type=3" \
  -F "station_id=7"
```

### 任务4: 烟雾判断B

```bash
curl -X POST http://localhost:3000/api/process \
  -F "image=@smoke_image.jpg" \
  -F "task_type=4" \
  -F "station_id=8"
```

### 任务5: 物品描述识别

```bash
curl -X POST http://localhost:3000/api/process \
  -F "image=@meter_test.jpg" \
  -F "task_type=5" \
  -F "station_id=5"
```

## 📂 项目结构说明

```
RaspiCartServer/
├── api_server/              # BentoML API服务（主要工作目录）
│   ├── service.py           # 服务入口
│   ├── config.yaml          # 配置文件
│   ├── base_project.py      # 基类定义
│   ├── loader.py            # 动态加载器
│   ├── utils.py             # 工具函数
│   ├── processors/          # 4个任务处理器
│   │   ├── task1_pointer_reader/
│   │   ├── task2_temperature/
│   │   ├── task3_smoke_a/
│   │   └── task4_smoke_b/
│   ├── start_server.py      # Python启动脚本
│   ├── start_server.bat     # Windows启动脚本
│   ├── start_server.sh      # Linux/Mac启动脚本
│   └── test_service.py      # 测试脚本
├── data/                    # 数据存储（运行时生成）
│   ├── images/              # 上传的图片
│   ├── logs/                # 日志文件
│   ├── database/            # 数据库（未来使用）
│   └── config/              # 配置文件
├── requirements.txt         # Python依赖
├── .env                     # 环境变量（需创建）
├── env.example              # 环境变量示例
└── README.md                # 项目说明
```

## 🔍 常见问题

### Q1: 启动时提示找不到模块

**解决方案**:
```bash
# 确保激活了虚拟环境
.\venv\Scripts\activate  # Windows
source venv/bin/activate # Linux/Mac

# 重新安装依赖
pip install -r requirements.txt
```

### Q2: 启动时提示 DASHSCOPE_API_KEY 未设置

**解决方案**:
- 检查 `.env` 文件是否存在
- 检查 `.env` 文件中是否正确设置了 `DASHSCOPE_API_KEY`
- 确保 `.env` 文件在项目根目录

### Q3: 调用API时返回错误

**可能原因**:
1. 图片格式不支持（只支持JPEG/PNG）
2. 图片太大（限制10MB）
3. task_type或station_id参数缺失
4. DashScope API密钥无效或额度不足

**检查方法**:
```bash
# 查看日志
tail -f data/logs/bentoml.log
```

### Q4: 服务启动成功但无法访问

**解决方案**:
- 检查防火墙是否阻止了3000端口
- 尝试访问 http://127.0.0.1:3000/health 而不是 localhost
- 检查端口是否被占用: `netstat -ano | findstr 3000` (Windows)

## 📊 性能指标

- **启动时间**: ~5秒
- **单次请求响应时间**:
  - 任务1（指针读数）: 2-5秒
  - 任务2（温度检测）: <1秒（本地处理）
  - 任务3/4（烟雾判断）: 2-5秒
  - 任务5（物品描述）: 2-5秒
- **并发能力**: 最多200个并发连接
- **超时设置**: 300秒

## 🔧 高级配置

### 修改端口

编辑 `api_server/config.yaml`:

```yaml
server:
  port: 3000  # 改为你想要的端口
```

### 调整并发数

编辑 `api_server/config.yaml`:

```yaml
server:
  workers: 4              # Worker进程数
  limit_concurrency: 200  # 最大并发连接数
```

### 禁用某个任务处理器

编辑 `api_server/config.yaml`:

```yaml
projects:
  - name: task1_pointer_reader
    enabled: false  # 设为false禁用
```

## 📝 日志查看

日志文件位置: `data/logs/bentoml.log`

实时查看日志:

```bash
# Windows (PowerShell)
Get-Content data\logs\bentoml.log -Wait

# Linux/Mac
tail -f data/logs/bentoml.log
```

## 🛠️ 开发模式

如果你需要修改代码：

1. 修改代码后，重启服务即可生效
2. 修改配置文件后，也需要重启服务
3. 添加新的任务处理器，参考 `api_server/架构说明.md`

## 📞 技术支持

- 查看详细文档: `README.md`
- 架构说明: `api_server/架构说明.md`
- 需求文档: `智能巡检服务端需求文档-服务端.md`

## 🎯 下一步

- 部署Web管理界面（Flask服务）
- 配置数据库（SQLite）
- 实现WebSocket实时推送
- 集成小车端程序

---

**祝你使用愉快！** 🚀

